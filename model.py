...
Targets: kills, deaths, hs_pct (headshot percentage)

Mirrors the LoL model.py exactly — same LightGBM setup, same chronological
train/val/test split, same SHAP analysis. Only the targets differ.
"""
...
DEFAULT_PARAMS = {
    "objective": "regression_l1",   # MAE loss — robust to outlier maps
    "metric": "mae",
    "learning_rate": 0.05,
    "num_leaves": 63,
...
def time_split(df, feature_cols, target, test_cutoff=None, val_cutoff=None):
    """Chronological train/val/test split — same as LoL model."""
    ...
    val_cutoff = val_cutoff or (min_date + pd.Timedelta(days=int(total_days * 0.60))).strftime("%Y-%m-%d")
    test_cutoff = test_cutoff or (min_date + pd.Timedelta(days=int(total_days * 0.80))).strftime("%Y-%m-%d")
...
def train_all(df, feature_cols, test_cutoff=None, val_cutoff=None, shap_n=300):
    all_metrics = {}

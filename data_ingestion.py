...
Then commit the CSV to your repo so Railway doesn't need to scrape.
"""
...
STATS_URL = (
    ...
    f"&rankingFilter=Top100&minMapCount=5"
)
...
def scrape_all_players() -> pd.DataFrame:
    ...
    try:
        log.info(f"[scrape] Fetching player stats for {START_DATE} to {END_DATE} (Top 100)")
...
def filter_top_teams(df: pd.DataFrame, min_maps: int = 5) -> pd.DataFrame:

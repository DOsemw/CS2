...
Kill Share architecture (mirrors LoL v2 model):
  ...
  Step 1: Look up player's individual KPR from dataset
  Step 2: Adjust for win probability and expected rounds
  Step 3: Player kills = KPR × expected_rounds
  Step 4: M1-2 = map1 × 2.0 (both maps guaranteed in Bo3)

Kill share is used to differentiate players within a team
but does NOT multiply on top of KPR (already baked in).
"""
...
def load_everything():
    ...
    # Kill share = player KPR / team avg KPR (for reference only, not multiplied)
...
def _shrinkage_blend(player_kpr: float, maps_played: float) -> float:
    """
    Bayesian shrinkage: blend player KPR toward global average
    for low-sample players. More maps = more trust.
    """
    if maps_played >= SHRINKAGE_THRESHOLD:
        return player_kpr
    w = maps_played / SHRINKAGE_THRESHOLD
    return w * player_kpr + (1 - w) * GLOBAL_AVG_KPR
...
def _predict(
    ...
    # Opponent adjustment — tougher opponents = fewer kills
    if opponent:
        opp_kpr = _lookup_team_avg_kpr(opponent)
        # Scale by how tough the opponent is relative to average
        opp_factor = (GLOBAL_AVG_KPR / max(opp_kpr, 0.3)) 
        opp_factor = min(max(opp_factor, 0.85), 1.15)  # cap at ±15%
        kpr_adj = kpr_adj * opp_factor

    # Map 1: KPR × expected rounds
    kills_map1  = round(kpr_adj * expected_rounds, 1)
    deaths_map1 = round(kills_map1 / max(kd_ratio, 0.3), 1)

    # M1-2 (Bo3) = exactly 2 maps

...
def load_everything():
    ...
    # Compute KPR = kills / (maps_played * 20.5 rounds avg)
    raw["kpr"] = (raw["kills"] / ROUNDS_COMPETITIVE).round(4)
    raw["kpr"] = raw["kpr"].clip(0.3, 1.2)

    # KD ratio — ensure it's there
...
def _expected_rounds(team_ml: int = None, opp_ml: int = None) -> float:
    """Return expected rounds based on moneyline intensity."""
...
def _win_prob(team_ml: int, opp_ml: int) -> float:
    ...
    total = p1 + p2
    return round(p1 / total, 3)


def _lookup_player(player: str) -> dict | None:
    """Find player in dataset, case-insensitive."""
    df = STATE["df"]
...
def _predict(player: str, team_ml: int = None, opp_ml: int = None,
    ...
    Returns per-map and M1-2 projections for kills and deaths.
    """
    ...
    if player_data is not None:
        ...
        log.warning(f"[predict] Player '{player}' not found — using fallback KPR {kpr}")

    # Win probability adjustment on KPR
    # If heavy favourite, slight boost; if underdog, slight reduction
    ...
    return _clean({
        "player":          player_data["playername"] if player_data else player,
        "team":            team,
        "win_prob":        win_prob,
        "kpr":             round(kpr, 4),

...
def load_everything():
    ...
    # Only train models that don't exist yet
...
def _load_model_safe(stat: str):
    """Load model if it exists, return None otherwise."""
...
def list_players(q: str = Query(None), team: str = Query(None)):
    ...
    return players.fillna("").to_dict(orient="records")
...
def predict_player(
    ...
    # Per-map predictions — skip missing models
    pm = {}
    for stat in TARGETS:
    ...
    # Filter to only stats with valid predictions for series projection
    ...
    bo1 = {stat: {k: v for k, v in vals.items()} for stat, vals in pm.items()}
    ...
    return {
        ...
        "team": player_df.iloc[0].get("team", ""),
